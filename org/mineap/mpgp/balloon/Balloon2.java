...
public class Balloon2 {
	 ...
	 */
	public Balloon2(ShellWindow sw, BalloonImage bi, String cName){
		this.bi = bi;
		this.sw = sw;
		this.cName = cName;
	...
	class Draw extends TimerTask{
		...
		public void run() {
			...
			if(sw.getIC().getTalkSyncQueue().peek() != null && sw.getIC().getTalkSyncQueue().peek() == myScope){
				...
				if(talkCountQueue.peek() != null && talkCountQueue.peek() <= talkNextCount){
					...
					//サーフェスの変更を指示する。出力文字数がmotionで指定された文字数より大きくなったときに実施。
		...
		/**
		 * 渡されたGraphicsインスタンスを使ってtalkArrayの内容を書き出す
		 * @param g2
		 */
		...
		/**
		 * 文字列の描画開始ポイントを設定、更新する
		 */
		private void setDrawStartPoint() {
			// TODO 自動生成されたメソッド・スタブ
			if(!isTalkComplete){
				if(!log.isEmpty() && log.size()+2>=(img.getHeight())/14){
					arrowFlag = true;
